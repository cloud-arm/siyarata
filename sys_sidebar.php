...
<body class="hold-transition skin-red skin-orange sidebar-mini " style="overflow-y: scroll;">
        ...
        <div class="row w-100">
            ...
            <div class="box box-success popup d-none" id="popup_1" style="width: 80%;">
                <div class="box-header with-border">
                    ...
                    <small class="btn btn-sm btn-info mx-2" style="padding: 5px 10px;" title="Sidebar Header" onclick="click_open(5)">Sidebar Header</small>
                            ...
                            <div class="col-md-3 sub1-sec sub2-sec sub-sec" style="display:none">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="main" onchange="select_main(this.value)" style="width: 100%;" autofocus>
                                        <?php
                                        $result = select("sys_sidebar", "*", "sub = 1 AND type = 'main' ORDER BY id DESC ");
                                        for ($i = 0; $row = $result->fetch(); $i++) {
                            ...
                            <div class="col-md-3 sub2-sec sub-sec" style="display:none">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="sub1" id="sub2" style="width: 100%;" autofocus>
                                        <?php
                                        $id = 0;
                                        $result = select("sys_sidebar", "*", "sub = 1 AND type = 'main' ORDER BY id DESC LIMIT 1");
                                        for ($i = 0; $row = $result->fetch(); $i++) {
                                            $id = $row['id'];
                                        }
                                        $result = select_query("SELECT * FROM sys_sidebar WHERE main_id = '{$id}' AND type = 'sub1' AND sub = 1 ORDER BY id DESC ");
                                        for ($i = 0; $row = $result->fetch(); $i++) {
                                            echo sprintf('<option value="%s"> %s </option>', $row['id'], ucfirst($row['name']));
                                        } ?>
                                    </select>
            ...
            <!-- header -->
            <div class="box box-success popup d-none" id="popup_5" style="width: 500px;">
                <div class="box-header with-border">
                    <h3 class="box-title w-100">
                        Sidebar Header
                        <i onclick="click_close(0)" class="btn btn-box-tool p-0 me-2 pull-right fa fa-remove" style="font-size: 25px"></i>
