...
if ($unit == 4) {
    ...
    $id = 0;
    $result = $db->prepare("SELECT id FROM sys_permission_arm WHERE menu_id='$menu' AND type = 'user_level' AND  user_level = '$level' ");
    $result->bindParam(':id', $menu);
    $result->execute();
    for ($i = 0; $row = $result->fetch(); $i++) {
        $id = $row['id'];
    ...
    if (!$id) {
        ...
        $sql = "UPDATE sys_permission_arm SET action = ? WHERE id = ?";
        $q = $db->prepare($sql);
...
if ($unit == 7) {
    ...
    $sec_head = 'name,type,action,order_id,head';
    $sec_act = ':name, :type, :action, :order_id, :head';
    $sec_val = ["name" => $name, "type" =>  "main", "action" => 1, "order_id" => $order_id, "head" => 1];
